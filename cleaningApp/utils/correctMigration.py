...
inputFolder = 'car-damage-detection/flaggedImages/'
outputFolder = 'car-damage-detection/scrappedImages/'
...
def processImage(data):
    ...
    copySource = {'Bucket': bucketName, 'Key': imageKey}
    if image.width < 450 or image.height < 450:
        s3Client.copy_object(Bucket=bucketName, CopySource=copySource, Key=f"{deleteFolder}{imageKey[len(inputFolder):]}")
        s3Client.delete_object(Bucket=bucketName, Key=imageKey)
        return
    ...
    if len(text) < 30:
        print(imageKey, 'to be back')
        s3Client.copy_object(Bucket=bucketName, CopySource=copySource, Key=f"{outputFolder}{imageKey[len(inputFolder):]}")
        s3Client.delete_object(Bucket=bucketName, Key=imageKey)
